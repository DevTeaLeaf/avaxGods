...
contract AVAXGods is ERC1155, Ownable, ERC1155Supply {
    ...
    uint public totalSupply; // Total number of tokens minted
    uint public constant DEVIL = 0;
    uint public constant GRIFFIN = 1;
    uint public constant FIREBIRD = 2;
    uint public constant KAMO = 3;
    uint public constant KUKULKAN = 4;
    uint public constant CELESTION = 5;

    uint public constant MAX_ATTACK_DEFEND_STRENGTH = 10;
    ...
    struct GameToken {
        ...
        uint id; /// @param id battle card token id; will be randomly generated
        uint attackStrength; /// @param attackStrength battle card attack; generated randomly
        uint defenseStrength; /// @param defenseStrength battle card defense; generated randomly
    }
    ...
    struct Player {
        ...
        uint playerMana; /// @param playerMana player mana; affected by battle results
        uint playerHealth; /// @param playerHealth player health; affected by battle results
    ...
    mapping(address => uint) public playerInfo; // Mapping of player addresses to player index in the players array
    mapping(address => uint) public playerTokenInfo; // Mapping of player addresses to player token index in the gameTokens array
    mapping(string => uint) public battleInfo; // Mapping of battle name to battle index in the battles array
    ...
    event NewGameToken(address indexed owner,uint id, uint attackStrength,uint defenseStrength);
    ...
    function _createRandomNum(uint _max,address _sender) internal view returns(uint randomValue) {
        uint randomNum = uint(keccak256(abi.encodePacked(block.difficulty, block.timestamp, _sender)));
    ...
    function _createGameToken(string memory _name) internal returns (GameToken memory) {
        ...
        uint8 randId = uint8(uint(keccak256(abi.encodePacked(block.timestamp, msg.sender))) % 100);
    ...
    function getBattleMoves(string memory _battleName) public view returns(uint P1Move,uint P2Move) {
    ...
    function _registerPlayerMove(uint _player,uint8 _choice,string memory _battleName) internal {
   ...
   function _awaitBattleResults(string memory _battleName) internal {
    ...
    function _resolveBattle(Battle memory _battle) internal {
        ...
        if (p1.move == 1 && p2.move == 1) {
            ...
            uint PHAD = p2.health + p2.defense;
            if (p1.attack >= PHAD) {
                _endBattle(_battle.players[0], _battle);
            } else {
                uint healthAfterAttack;
            ...
            uint PHAD = p1.health + p1.defense;
            if (p2.attack >= PHAD) {
                _endBattle(_battle.players[1], _battle);
            } else {
                uint healthAfterAttack;
        ...
        uint _randomAttackStrengthPlayer1 = _createRandomNum(MAX_ATTACK_DEFEND_STRENGTH, _battle.players[0]);
        ...
        uint _randomAttackStrengthPlayer2 = _createRandomNum(MAX_ATTACK_DEFEND_STRENGTH, _battle.players[1]);
